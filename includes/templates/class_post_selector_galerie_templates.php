...
class Post_Selector_Galerie_Templates {
	/**
	...
	public function loadGalerieTemplate($attributes)
	{
		...
		$galerie = apply_filters($this->basename.'/post_selector_get_galerie', $args, false);
		if (!$galerie->status) {
			return null;
		}
	...
	private function render_galerie_slider_template($galerieSettings, $typeSettings, $images, $count, $attributes): string
	{
		...
		foreach ($images as $tmp):
			...
			$attachment = apply_filters($this->basename.'/post_selector_wp_get_attachment', $tmp->img_id);
